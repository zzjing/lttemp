public class Python_Indentation_Validation {
    public boolean validate(String[] lines) {
        ...
        for (String line : lines) {
            int blockLevel = getIndentNum(line); // every step needs to check how many indentation
            // 1. check first line, stack is initially empty
            if (codeBuffer.isEmpty()) {
                if (blockLevel != 0) { // first line, must be 0 indents
                    // print out line
                    return false;
                }
            } else if (codeBuffer.peek().charAt(codeBuffer.peek().length() - 1) == ':') { // 2. ends with control :
                if (blockLevel != getIndentNum(codeBuffer.peek()) + 1) { // must be one indent more than last line's
                    // print out line
                    return false;
                }
            } else {
                // 3. if one "nested" statement block finishes, pop all nested lines with more indents
