...
public class OnBoardWordSearch {
    ...
    private boolean dfsBoard(char[][] board, String word, int i, int j, int nextChar) {
        if (nextChar == word.length()) {
            return true; // found the word since all previous char search returns true
        }
        if (i < 0 || i >= board.length || j < 0 || j >= board[0].length
            || board[i][j] != word.charAt(nextChar)) {
            return false;
        }
        board[i][j] = '#'; // mark the current position as visited/matched by "#"
        // continue searching the next character(s) toward every direction
        boolean found = dfsBoard(board, word, i - 1, j, nextChar + 1) ||
            dfsBoard(board, word, i + 1, j, nextChar + 1) ||
            dfsBoard(board, word, i, j - 1, nextChar + 1) ||
            dfsBoard(board, word, i, j + 1, nextChar + 1);
        ...
        board[i][j] = word.charAt(nextChar); // we can get it back
